...
test.describe('CustomerVideobookingcreation', () => {
  test('@regression create video booking and verify in My Bookings', async ({ page, loginPage }) => {
    ...
    await test.step('Open booking form, select DigitalTolk video service, and fill required details', async () => {
      await createBookingPage.gotoInterpreterBookingForm();
      const result = await createBookingPage.fillVideoBookingBasics('Ludhianiska');
      selectedLanguage = result.selectedLanguage;
      durationMinutes = result.durationMinutes;
      await createBookingPage.goToPreview();
    });
    ...
    await test.step('Create booking and capture booking ID', async () => {
      ...
      if (!createdBookingId) {
        throw new Error('Booking ID is empty after video booking creation.');
      }
    });
