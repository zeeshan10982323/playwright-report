...
test.describe('CustomerPhonebookingcreation', () => {
  test('@regression create phone booking and verify in My Bookings', async ({ page, loginPage }) => {
    ...
    await test.step('Open phone booking form and fill required details', async () => {
      await createBookingPage.gotoInterpreterBookingForm();
      const result = await createBookingPage.fillPhoneBookingBasics('Ludhianiska');
    ...
    await test.step('Create booking and capture booking ID', async () => {
      ...
      if (!createdBookingId) {
        throw new Error('Booking ID is empty after booking creation.');
      }
    });
