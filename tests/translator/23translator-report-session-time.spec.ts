...
test.describe('@regression translator report session time', () => {
  test('translator can report session with a comment', async ({ page, loginPage }) => {
    ...
    for (let i = 0; i < maxTries; i += 1) {
      ...
      const reportSessionBtn = dialog
        .getByRole('button', { name: /report session/i })
        .or(dialog.locator('button, a').filter({ hasText: /report session/i }))
        .first();

      if (await reportSessionBtn.isVisible().catch(() => false)) {
        chosenRowIndex = i;
        await reportSessionBtn.click({ force: true });
        break;
      }
    ...
    const commentBox = page
      .locator('textarea.n-input__textarea-el[placeholder="Enter comment"]')
      .first();
    await expect(commentBox).toBeVisible({ timeout: 20000 });
    await commentBox.fill('test');
    await wait(300);

    const submitBtn = page
      .getByRole('button', { name: /submit/i })
      .or(page.locator('button').filter({ hasText: /submit/i }))
      .first();
