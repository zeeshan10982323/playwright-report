...
test.describe('@regression translator good feedback', () => {
  test('translator can give positive feedback on a completed booking', async ({ page, loginPage }) => {
    ...
    const fifthStar = stars.nth(Math.min(4, starCount - 1));
    await fifthStar.click({ force: true });
    await wait(500);

    const friendlyOption = feedbackPanel
      .locator('.feedback-option-row .base-text')
      .filter({ hasText: /the customer user was friendly/i })
      .first();
    ...
    const thoughtsTextarea = feedbackPanel
      .locator('textarea, [placeholder*="write down your thoughts" i]')
      .first();
    if (await thoughtsTextarea.isVisible().catch(() => false)) {
      await thoughtsTextarea.fill('Great');
      await wait(500);
    }
    ...
    expect(dialogText).toContain('the customer user was friendly');
    expect(dialogText).toContain('great');
  });
});
